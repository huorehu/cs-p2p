...
public class ArchiveCaptionWriter {
    ...
    public ArchiveCaptionWriter(BitsBufferedWriter writer, AbstractCoder coder) {
    ...
    public byte[] toByteArray(long data, int bytesAmount) {
        ...
        for (int i = 0; i < bytesAmount; i++) {
            ...
            dataPart = (byte) (data >> BITS_IN_BYTE * (bytesAmount - 1) - i * BITS_IN_BYTE);
            result[i] = dataPart;
        }
        return result;
    }
     ...
     *
     * @param coder contains codes for writing to table.
     */
    private byte[] initTableSize(AbstractCoder coder) {
        ...
        int reservedBytesForOriginByteAndAmountOfBitsForCode = 2;
        ...
        int tableSizeInt = reservedBytesForOriginByteAndAmountOfBitsForCode * coder.getPowerAlphabet() + bytesForCodes;
     ...
     * @param coder contains codes for writing to table.
     */
    private byte[] initDataSize(AbstractCoder coder) {
        return toByteArray(coder.getSrcDataSize(), DATA_SIZE_BYTES);
    }

    /** Writes data of caption to buffer */
    private void initCaption(BitsBufferedWriter writer, AbstractCoder coder) {
        try {
            writer.addCode(tableSize);
            writer.addCode(dataSize);

            // Writes a byte-bit sequence association table.
            for (MapP2P.Node<Byte, String> entry : coder.getCodesAlphabet().entrySet()) {
